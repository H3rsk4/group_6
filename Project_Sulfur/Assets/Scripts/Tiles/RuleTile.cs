...
namespace UnityEngine
{
    ...
    public class RuleTile : TileBase
    {
            ...
            if (m_TilingRules != null && m_TilingRules.Count > 0)
            {
                for (int y = -1; y <= 1; y++)
                {
                    for (int x = -1; x <= 1; x++)
                    {
                        /*
                        if(x != 0 && y != 0){
                            Vector3 worldPos = tileMap.GetComponent<Transform>().parent.GetComponent<tile_manager>().maps[1].CellToWorld(new Vector3Int(location.x, location.y, location.z) + new Vector3Int(x,y,0));
                            tile_manager newTileManager = tilemanager_from_position.GetTileManager(worldPos);
                            if(newTileManager != null){
                                //Debug.Log(newTileManager);
                                Vector3Int newCellPos = newTileManager.maps[1].WorldToCell(worldPos);
                                TileBase tile = newTileManager.maps[1].GetTile(newCellPos);
                                if(tile is RuleTile){
                                    RuleTile newTile = (RuleTile)tile;
                                    newTile.RefreshTile(newCellPos, newTile.m_ITilemap);
                                }
                            }
        ...
        public virtual bool RuleMatch(int neighbor, TileBase other)
        {
            switch (neighbor)
            {
                case TilingRule.Neighbor.This: return other == m_Self;
                case TilingRule.Neighbor.NotThis: return other != m_Self;
        ...
        protected virtual void GetMatchingNeighboringTiles(ITilemap tilemap, Vector3Int position, ref TileBase[] neighboringTiles)
        {
            ...
            for (int y = 1; y >= -1; y--)
            {
                for (int x = -1; x <= 1; x++)
                {
                    if (x != 0 || y != 0)
                    {
                        Vector3Int tilePosition = new Vector3Int(position.x + x, position.y + y, position.z); //cellposition
                        //Debug.Log(tilePosition);
                        TileBase tile = tilemap.GetTile(tilePosition);
                        m_CachedNeighboringTiles[index++] = tile;
                        if(tile == null){
                            //find other tilemanager tilemaps and tilebases
                            // Vector3 worldPos = tilemap.CellToWorld(tilePosition);
                            //Debug.Log(tilemap.GetComponent<Transform>().parent.GetComponent<tile_manager>().maps[1].CellToWorld(tilePosition));
                            Vector3 worldPos = tilemap.GetComponent<Transform>().parent.GetComponent<tile_manager>().maps[1].CellToWorld(tilePosition);
                            tile_manager newTileManager = tilemanager_from_position.GetTileManager(worldPos);
                            if(newTileManager != null){
                                //Debug.Log(newTileManager);
                                Vector3Int newCellPos = newTileManager.maps[1].WorldToCell(worldPos);
                                tile = newTileManager.maps[1].GetTile(newCellPos);
                                if(tile != null){
